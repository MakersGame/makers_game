...
func update_items_in_backpack():
    ...
    for key in Global.GoodInBackpack.keys():
        if Global.GoodInBackpack[key]>0 and (ItemType=="All" or ItemType==ReferenceList.ItemRference[key]["Type"]):
...
func page_change_action():
    ...
    for Inventory in Inventories:
        if Inventory.Number+4*(CurrentPage-1)<=SequencedItem.size():
            Inventory.update_sprite(SequencedItem[4*(CurrentPage-1)+Inventory.Number-1],-1)
        elif Inventory.Number+4*(CurrentPage-1)<=SequencedItem.size()+SequencedWeapon.size():
            Inventory.update_sprite(Global.WeaponInBackpack[SequencedWeapon[4*(CurrentPage-1)+Inventory.Number-1-SequencedItem.size()]]["Name"],SequencedWeapon[4*(CurrentPage-1)+Inventory.Number-1-SequencedItem.size()])
        else:
...
func item_detail_change():
    if InventoryChosen==0 or 4*(CurrentPage-1)+InventoryChosen>SequencedItem.size()+SequencedWeapon.size():
        ...
        $ItemDetail/ItemName.text=Global.WeaponInBackpack[SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()]]["Name"]
        $ItemDetail/ItemType.text=ReferenceList.WeaponReference[Global.WeaponInBackpack[SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()]]["Name"]]["Type"]
        $ItemDetail/ItemDescription.text=ReferenceList.WeaponReference[Global.WeaponInBackpack[SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()]]["Name"]]["Description"]
        $ItemDetail/ItemDescription.text+="\n耐久："+String(Global.WeaponInBackpack[SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()]]["Durability"])+"/"
        $ItemDetail/ItemDescription.text+=String(ReferenceList.WeaponReference[Global.WeaponInBackpack[SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()]]["Name"]]["MaxDurability"])
        $ItemDetail.show()
...
func _on_Drop_pressed():
    ...
    if 4*(CurrentPage-1)+InventoryChosen<=SequencedItem.size():
        $ItemDetail/DropItemWindow.init(Global.GoodInBackpack[SequencedItem[4*(CurrentPage-1)+InventoryChosen-1]])
...
func _on_DropItemWindow_DropItem():
    FocusChangable=true
    Global.GoodInBackpack[SequencedItem[4*(CurrentPage-1)+InventoryChosen-1]]-=$ItemDetail/DropItemWindow.DropNumber
...
func _on_DropWeapon_OK():
    FocusChangable=true
    Global.WeaponInBackpack.remove(SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()])
    Global.update_pause_window()
...
func _on_AccessItemWindow_OK():
    ...
    if Global.GoodInHome.get(ItemName)==null:
...
func _on_MoveToHome_pressed():
    if !FocusChangable and !$ItemDetail/AccessItemWindow.visible and !$ItemDetail/AccessWeaponWindow.visible:
        return
    FocusChangable=false
    if 4*(CurrentPage-1)+InventoryChosen<=SequencedItem.size():
        $ItemDetail/AccessItemWindow.init(Global.GoodInBackpack[SequencedItem[4*(CurrentPage-1)+InventoryChosen-1]])
        $ItemDetail/AccessItemWindow.show()
    else:
        FocusChangable=true
        var WeaponNum=SequencedWeapon[4*(CurrentPage-1)+InventoryChosen-1-SequencedItem.size()]
        var WeaponInfo=Global.WeaponInBackpack[WeaponNum]
        Global.WeaponInBackpack.remove(WeaponNum)
        Global.WeaponInHome.push_back(WeaponInfo)
        Global.update_pause_window()
